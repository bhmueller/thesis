...
# Until PR is through:
from morris import _shift_cov
from morris import _shift_sample
from morris import _uniform_to_standard_normal
...
def test_linear_function_a():
    ...
    # Assert sigma_corr
    assert (
        ee["sigma_corr"].loc["x3"]
        < ee["sigma_corr"].loc["x2"]
        < ee["sigma_corr"].loc["x1"]
    )
