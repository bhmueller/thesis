...
seem to converge as $N$ increases. \Cref{rel_diff_morris} gives details on the convergence. \citet{ASAV19} show for the classical independent Morris indices that they do indeed converge. Since in the end, the full Morris indices are estimated in a similar fashion, I do not see why full indices should not converge. Unfortunately \textit{full} Morris indices do not converge for all replicates as noted in \cref{morris_replicates} and supported further by \cref{accuracy}. In fact, the share of correct rankings steadily declines as $N$ increases. I investigate whether this behaviour can be observed for a larger sample size. I consider the replicate of Morris indices with $N=500$ which shows the largest difference $\mu^{\ast,\ full}_{RC} - \mu^{\ast,\ full}_{\theta_{11}}$ and re-estimate the full indices for larger sample sizes, i.e. for $N \in \{3\,000,\ 4\,000,\ 6\,000,\ 8\,000\}$. Details are shown in \cref{appendix_a} in \cref{appendix_morris_seed_67}. There Morris indices seem to converge, but still yield the wrong input ranking. Unfortunately due to the computational limitations I face, I cannot investigate this further.
